# Model Card: Financial Stability Transition

Purpose: evidence-bounded target-trial transition for earnings, employment, income support, and financial-security outcomes.

Estimand: average treatment effect for an advisor-reviewed intervention prior whose outcome_domain is `financial_stability`.
...
Validation status: labor-market and AI-exposure context can calibrate baseline risk, but no headline result may use a single exposure score as a causal effect.
